...
int main(int argc, char **argv) {
  ...
  // process with rank 0 will divide data among all processes and add partial
  // sums to get final sum
  if (rank == 0) {
    int index, i;
    ...
    if (num > 1)  // if more than 1 processes available
    {
      ...
      for (i = 1; i < num - 1; i++) {
        // calculating first index of subarray that need to be send to ith
        // process
    ...
    printf("Sum of array = %d\n", sum);
          ...
          // partial sums
  {
    // receive no of elements and elements form process 0 and store them on
    // local array
