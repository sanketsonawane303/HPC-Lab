...
int main(int argc, char *argv[]) {
  ...
  if (rank == 0) {
    ...
    for (dest = 1; dest <= numworkers; dest++) {
      ...
      MPI_Send(&a[offset][0], rows * noOfColA, MPI_DOUBLE, dest, tag,
               MPI_COMM_WORLD);
    ...
    for (i = 1; i <= numworkers; i++) {
      ...
      MPI_Recv(&c[offset][0], rows * noOfColB, MPI_DOUBLE, source, tag,
  ...
  if (rank > 0) {
    ...
    MPI_Recv(&b, noOfColA * noOfColB, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD,
             &status);
